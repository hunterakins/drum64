...
volatile uint16_t audio_buffer_counter = AUDIO_BUFFER_SIZE - 16;
...
uint16_t generate_sample(DrumCounters *dcounters) {
  ...
  uint16_t instrument_sample;
  long double amplitude_multiplier;
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (dcounters->drum_playing[i]) {
      ...
      instrument_sample = samples[i][dcounters->drum_counter[i]] - 0x7FF;

      if (i == 4) {
        instrument_sample = instrument_sample / 4;
      }

      /*
      //chprintf((BaseSequentialStream *)&SD1, "PRE MULTIPLIER %d\r\n", instrument_sample);
      amplitude_multiplier = (dcounters->drum_amplitude_Y[i]);
      //chprintf((BaseSequentialStream *)&SD1, "AMPLITUDE RAW %d\r\n", amplitude_multiplier);

      amplitude_multiplier *= 0.004;
      ...
      instrument_sample = (uint16_t) (instrument_sample * amplitude_multiplier);
      //chprintf((BaseSequentialStream *)&SD1, "POST MULTIPLIER %d\r\n", instrument_sample);
      */
      instant_sample += instrument_sample;
    }
  }
...
int main(void) {
  ...
  for (int i = 0; i < 8; i++) {
    dcounters.drum_playing[i] = 1;
    dcounters.drum_amplitude_X[i] = 127;
    dcounters.drum_amplitude_Y[i] = 127;
  ...
  while (1) {
    ...
    if (counter % POLL_PERIOD == 0) {
      controller_state = read_poll_signal_debug();
