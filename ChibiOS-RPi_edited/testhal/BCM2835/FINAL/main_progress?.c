...
#define SAMPLERATE 11025  // in Hz
#define POLL_PERIOD 8// 100 // 8 // number of samples per poll
#define POLL_READ   3//50 // 3 // number of samples in, to send read signal
...
/*
 * writes sample to GPT .
 * SHOULD send poll signal at every x'th sample
 * AND ...! delay writing samples till later ...
 * 1000000 / SAMPLERATE
 */
...
void write_sample(GPTDriver *gptp) { // takes about 3us of computation
  ...
  if ((counter - POLL_READ) % POLL_PERIOD == 0) {
   ...
   // for some reason, tryin to clear the ra bits KILLS it
    // disable I2C really quickly ...
    // set bits to WRITE


    // find a way to CLEAR STATUs
    //device->status = BSC_CLKT | BSC_ERR | BSC_DONE;
    /*
    mem = BSC0 + 0x04; // STATUS
    ra = GET32(mem);
    //ra = 0;
    ra |= (BSC_CLKT | BSC_ERR | BSC_DONE);
    //ra &= ~(1 << 15); // disable FIFO really quickly ...
    PUT32(mem, ra);*/
    /*
    mem = BSC0; // STATUS
    ra = GET32(mem);
    ...
    */
    ...
    // increase increment a littttle bit?
    ...
    if ((counter >= 3*AUDIO_BUFFER_SIZE) & (counter < 4*AUDIO_BUFFER_SIZE)) {
    ...
    mem = BSC0; // STATUS
    ra = GET32(mem);
    ra |= START_WRITE; // (BSC_INTT | BSC_INTD | START_WRITE); // we don't need interrupt
    PUT32(mem, ra);
