...
#define POLL_PERIOD 100 // 8 // number of samples per poll
#define POLL_READ   50 // 3 // number of samples in, to send read signal
...
int main(void) {
  ...
  while (1) {
    ...
    if (counter % POLL_PERIOD == 0) {
      ...
      if (controller_state != (~(0))) { // if without bugs
